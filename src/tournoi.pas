...
BEGIN
  ...
  For i := 0 TO x-1 DO BEGIN
    FOR j := i+1 TO x-1 DO begin
      ...
      WRITE('Nombre de buts marque par ',classement.noms[i],' : ');
      READLN(e1);

      WRITE('Nombre de buts marque par ',classement.noms[j],' : ');
      READLN(e2);
      WRITELN('');

      IF e1 > e2 THEN
        classement.points[i] := classement.points[i] + 3;
      IF e2 > e1 THEN
        classement.points[j] := classement.points[j] +3;
      IF e1 = e2 THEN BEGIN
...
BEGIN
    ...
    WHILE (i <= j) DO BEGIN
      WHILE (i <= fin) AND (classement.points[i] >= pivot) DO
        i := i+1;
      WHILE (j > debut) AND (classement.points[j] < pivot) DO
      j := j-1;
      IF (i<j) THEN BEGIN
...
BEGIN
  ...
  FOR i := 0 TO x-2 DO BEGIN
    ...
    IF (classement.points[i] = classement.points[j]) THEN BEGIN
      ...
      IF diff2 > diff1 THEN
        changeOrdre(classement,i,j)
      ELSE IF buts2 > buts1 THEN
        changeOrdre(classement,i,j)
      ELSE BEGIN
        REPEAT
...
BEGIN
  ...
  FOR i := 0 to x-1 DO BEGIN
    ...
    IF (lmax < length(classement.noms[i])) THEN
      lmax := length(classement.noms[i]);
  END;
